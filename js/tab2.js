...
class Tab {
	...
	// 클래스에 등록한 변수 앞쪽에 #을 붙이면 private 설정으로 변경되고 private로 등록된 변수는 외부에서 변경 불가능한 캡슐화 상태
	...
	// tab이라는 property에 key에 접근하는 순간 호출되는 함수
	get tab() {
		return this.value;
	}

	// tab이라는 property value에 값을 담으려는 순간 호출되는 함수
	set tab(value) {
