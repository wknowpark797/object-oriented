...
function FontStyle(el, option) {
	...
	/*
    일반 이벤트가 연결되어 있는 function문 안쪽에서 this 값이 이벤트가 발생한 선택자로 변경된다.
		-> 때문에 해당 값을 해당 function문 외부에서 bind(this)로 고정

    [ 일반 함수에서 이벤트 사용 시 이슈 발생 ]
      이벤트 안쪽에서 this는 클릭한 요소를 가리킨다.
      때문에 생성자 함수에 저장된 프로토타입의 함수를 불러오지 못한다.
      -> bind 사용
  */
}
